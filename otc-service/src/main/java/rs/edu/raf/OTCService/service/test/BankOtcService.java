...
//    private static final String URL_TO_BANK3 =  "http://localhost:8083/api/v1/otcTrade";
    ...
    public List<MyStockDto> findAllStocks() {
        List<SecurityOwnershipDto> banksStocks = bankService.getSecurityOwnerships();
        myStockRepository.deleteAll();
        List<MyStockDto> dtos = new ArrayList<>();
        for (SecurityOwnershipDto stock : banksStocks) {
            MyStockDto dto = new MyStockDto();
            dto.setAmount(stock.getQuantityOfPubliclyAvailable());
            dto.setTicker(stock.getSecuritiesSymbol());
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    public Offer receiveOffer(OfferDto offerDto) {
        ...
        Optional<SecurityOwnershipDto> myStock = bankService.getSecurityOwnerships().stream().filter(stock -> stock.getSecuritiesSymbol().equals(offer.getTicker())).findFirst();

        //provera da li mi imamo taj Stock
        if (myStock.isPresent() && myStock.get().getQuantityOfPubliclyAvailable() >= offer.getAmount() && offer.getAmount() >= 0) {
    ...
    public boolean offerAccepted(Long id) {
        ...
        if (myOfferOptional.isPresent()) {
            ...
            bankService.buyBank3Stock(new MyOfferDto(null, myOffer.getTicker(), myOffer.getAmount(), myOffer.getPrice()));
    ...
    public boolean acceptOffer(Long id) {
        ...
        if (offer.isPresent()) {
            ...
            bankService.sellStockToBank3(new OfferDto(offer1.getTicker(), offer1.getAmount(), offer1.getPrice(), offer1.getIdBank()));
