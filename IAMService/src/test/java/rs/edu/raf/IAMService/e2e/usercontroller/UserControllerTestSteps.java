...
public class UserControllerTestSteps extends UserControllerTestConfig{
    ...
    public void user_send_req(){
        String url = "http://localhost:8000/api/users/password-change";

        ResultActions resultActions =  mockMvc.perform(get("/api/users/password-change")
                .contentType(MediaType.APPLICATION_JSON_VALUE)
    ...
    public void response_status(){
        //assert entity == null;
        //assert entity.getBody() == Boolean.TRUE;
    }

    //create employee
    @Given("user gives valid paramethers for employee email {string} and id {string}")
    public void user_gives_valid_paramethers_for_employee_email_and_id(String string, String string1) {
       employeeDto.setEmail(string);
       employeeDto.setId(Long.valueOf(Integer.parseInt(string1)));
       id = Long.valueOf(Integer.parseInt(string1));
    }
    @When("user send request")
    public void user_send_request() throws Exception {
        String url = "http://localhost:8000/api/users/create/employee";
        ResultActions resultActions =  mockMvc.perform(get("/api/users/create/employee")
                .contentType(MediaType.APPLICATION_JSON_VALUE)
    ...
    @Given("user gives valid paramethers for agent email {string} and id {string}")
    public void userGivesValidParamethersForAgentEmailAndId(String arg0, String arg1) {
        agentDto.setEmail(arg0);
        employeeDto.setId(Long.valueOf(Integer.parseInt(arg1)));
        id = Long.valueOf(Integer.parseInt(arg1));
    }

    @SneakyThrows
    @When("user send request to create agent")
    public void userSendRequestToCreateAgent() {
        ResultActions resultActions =  mockMvc.perform(get("/api/users/create/agent")
                .contentType(MediaType.APPLICATION_JSON_VALUE)
